        ...
        while (cmsg != NULL) {
            if (cmsg->cmsg_level == SOL_SOCKET) {
                ...
                if (cmsg->cmsg_type == SCM_TIMESTAMPNS) {
                    ...
                    // Convert to CPU cycles (approximate)
                    ...
                    if (nic_ns <= now_ns && now_ns > 0) {
                        ...
                        ctx->last_nic_timestamp_ticks = now_ticks;
                    }
                    break;
                }
                #endif
...
int ssl_connect(SSLContext* ctx, int fd, const char* hostname) {
    ...
    if (ctx->backend == SSL_BACKEND_SECURETRANSPORT) {
        ...
        // Perform initial handshake attempt (non-blocking)
        status = SSLHandshake(ctx->st_ctx);
        if (status == noErr) {
            return 0;  // Handshake complete
        }
        if (status == errSSLWouldBlock) {
            // Would block - handshake will continue asynchronously via I/O
            // Return success, caller will retry via ssl_read/ssl_write
            return 0;
        }
